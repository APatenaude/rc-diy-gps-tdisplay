...
void ublox_noNMEA() {
  byte messages[][2] = {
      {0xF0, 0x0A}, {0xF0, 0x09}, {0xF0, 0x00}, {0xF0, 0x01}, {0xF0, 0x0D},
      {0xF0, 0x06}, {0xF0, 0x02}, {0xF0, 0x07}, {0xF0, 0x03}, {0xF0, 0x04},
      {0xF0, 0x0E}, {0xF0, 0x0F}, {0xF0, 0x05}, {0xF0, 0x08}, {0xF1, 0x00},
      {0xF1, 0x01}, {0xF1, 0x03}, {0xF1, 0x04}, {0xF1, 0x05}, {0xF1, 0x06},
  };
...
void ublox_changeFrequency15() {
  byte packet[] = {
      0xB5, 0x62, 0x06, 0x08, 0x06, 0x00, 0x42,
      0x00, 0x01, 0x00, 0x01, 0x00, 0x58, 0x46,
  };
  ublox_sendPacket(packet, sizeof(packet));
}
...
void ublox_turnOff() {
  ...
  byte packet[] = {
      0xB5, 0x62, 0x06, 0x57, 0x08, 0x00, 0x01, 0x00,
      0x00, 0x00, 0x50, 0x4F, 0x54, 0x53, 0xAC, 0x85,
  };
  ublox_sendPacket(packet, sizeof(packet));
}
...
void ublox_turnOn() {
  ...
  byte packet[] = {
      0xB5, 0x62, 0x06, 0x57, 0x08, 0x00, 0x01, 0x00,
      0x00, 0x00, 0x20, 0x4E, 0x55, 0x52, 0x7B, 0xC3,
  };
  ublox_sendPacket(packet, sizeof(packet));
}
...
void setup() {
  ...
  displayInit();
  voltageReadInit();
  xTaskCreate(battery_info, "battery_info", 4096, NULL, 1, NULL);
  ...
  while (true) {
    if (!screenOff) {
      ...
      if (vbat >= MIN_USB_CHRG_VOL) {
        ...
        int imgNum = 0;
        int batteryLevel = getChargeLevel(vbat);
        if (batteryLevel >= 80) {
          imgNum = 3;
...
void drawingText(String text) {
  ...
  tft.setTextDatum(5);
  tft.drawString(text, ICON_POS_X - 5, STATUS_HEIGHT_BAR / 2, 4);
}
...
void button_init() {
  ...
  btn2.setClickHandler([](Button2 &b) { toggleScreen(); });
  btn2.setLongClickHandler([](Button2 &b) { toggleScreen(); });
}
